...
DB_ARCHIVE = os.path.join(APP_DIR, "pcards.db.gz")
DB_SOURCE = os.path.join(APP_DIR, "pcards.db")
DB_PATH = os.path.join(tempfile.gettempdir(), "pcards-audit.db")
...
def prepare_database():
    ...
    if os.path.exists(DB_ARCHIVE):
        with gzip.open(DB_ARCHIVE, "rb") as source, open(DB_PATH, "wb") as target:
            shutil.copyfileobj(source, target)
    elif os.path.exists(DB_SOURCE):
        shutil.copyfile(DB_SOURCE, DB_PATH)
    else:
        raise FileNotFoundError(
            "pcards.db.gz or pcards.db is missing from the application folder."
        )
...
def question_to_sql(question, year):
    ...
    if not key:
        raise RuntimeError(
            "The Gemini API key has not been configured in the website secrets."
        )
...
"""
    ...
    if not response.text:
        raise RuntimeError("Gemini did not return a query.")
    return validate_read_only_sql(response.text)
...
try:
    ...
    st.error(f"The transaction database could not be prepared: {error}")
    st.stop()
